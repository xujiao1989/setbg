    ...
    var popMethod = {
        ...
        __initHtml: function (option) {
            var style = option.style,
                ...
                popHead = option.hasheader == 0 ? '<div class="pop_header" style="' +
                style.pop_header + '">' +
                '<a href="javascript:void(0)" class="close_pop_btn" style="' + style.close_btn + '">关闭</a>' +
                '<p class="pop_title" style="' + style.pop_title + '">' + option.title + '</p>' +
                '</div>' :
                '',
                popContainer = '<div class="pop_container" style="' + style.pop_container + '">' + '', popMain = option.type == '0' ? '<div class="pop_main_con" ><iframe scrolling="no" id="setbg_iframe" src="' + option.url + '" frameborder="0" ></iframe></div></div>' : '<div class="pop_main_con" style="overflow: hidden">' + option.html + '</div></div>';
        ...
        __resizePop: function (opt) {
            var self = this,
                ...
                _headerHeight =_header &&  _header.height() ? _header.height() : 0,
                width = opt.width,
                height = opt.height,
        ...
        __initPopSize: function (option) {
            var type = option.type,
                ...
                self = this,
                popCon = $('.pop_main_con'),
                userHeight = option.height,
                userWidth = option.width;
            ...
            iframe.on('load', function () {
                var iframeWidth = Math.max(iframe.contents().find('body').width(),iframe.contents().find('html').width()),
                    iframeheight = Math.max(iframe.contents().find('body').height(),iframe.contents().find('html').height());
